...
panelApi.interceptors.request.use(async (configReq) => {
  try {
    const settings = await getStoreSettings();
    if (settings.panel_base_url) {
      configReq.baseURL = settings.panel_base_url;
    }
    if (settings.panel_api_key) {
      configReq.headers['x-api-key'] = settings.panel_api_key;
    }
  } catch (error) {
    console.error('[PanelClient] Failed to fetch settings', error);
  }
  return configReq;
});
